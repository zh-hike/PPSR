...
def log_train_info(engine, epoch_id, iter_id):
    ...
    ips = int(engine.cfg['Data']['Train']['DataLoader']['batch_sampler']['batch_size'] * 2 // engine.time_info['batch_cost'].avg)
    ...
    info = f"Train: Epoch: [{epoch_id}/{engine.cfg['Global']['epochs']}]  Step: [{iter_id}/{engine.cfg['Global']['step_per_epoch']}]  lr: {lr_info}  {batch_cost} {read_cost} {loss} ips: {ips}/s  eta: {train_estimate_time}"
